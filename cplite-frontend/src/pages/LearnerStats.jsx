...
const LearnerStats = ({ user, API_URL }) => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      setLoading(true);
      try {
        // Fetch stats and summary data from database
        const statsResponse = await axios.get(
          `${API_URL}/ai/db/user/${user.handle}/stats`
        );
        
        const summaryResponse = await axios.get(
          `${API_URL}/ai/db/user/${user.handle}/summary`
        );
        ...
        if (err.response && err.response.status === 404) {
          setError("No statistics available in database.");
        } else {
          setError("Failed to load your progress statistics. Please try again later.");
        }
      } finally {
        setLoading(false);
      }
    ...
    if (user?.handle) {
      ...
      setError("CodeForces handle not found. Please update your profile.");
      setLoading(false);
    }
  }, [user, API_URL]);
  ...
  if (!stats || Object.keys(stats).length === 0) {
    return (
      <div style={styles.emptyState}>
        ...
        <p>
          There are no statistics available in the database. Please contact an administrator.
        </p>
      </div>
    );
  }
  ...
  return (
    <div style={styles.container}>
      <div style={styles.statsHeader}>
        <h2>Your Problem Solving Progress</h2>
      ...
      <div style={styles.summarySection}>
        ...
        <div style={styles.summaryBox}>
          {summary ? (
            ...
            <p>No personalized summary available.</p>
          )}
        </div>
      </div>
